...
export class CodeCell extends Cell {
  ...
  updateContent(content: string): CodeCell {
    return new CodeCell(content, this._output, this.id, this.createdAt, new Date(), this.isEditing);
  }

  setEditing(isEditing: boolean): CodeCell {
    return new CodeCell(this.content, this._output, this.id, this.createdAt, new Date(), isEditing);
  }

  withOutput(output: CellOutput): CodeCell {
    return new CodeCell(this.content, output, this.id, this.createdAt, new Date(), this.isEditing);
  }

  clearOutput(): CodeCell {
    return new CodeCell(this.content, undefined, this.id, this.createdAt, new Date(), this.isEditing);
  }
}
